...
impl TransactionalSkillCreator {
    ...
    pub fn management_status(
        ...
        match fs::read_to_string(metadata_path) {
            Ok(value) => match ManagedSkillMetadata::from_json(&value) {
                Ok(metadata) => match self.managed_content(name)? {
                    Some(content) if metadata.has_matching_content_digest(&content) => {
                        Ok(Some(ManagedSkillStatus::Managed(metadata)))
                    }
                    _ => Ok(Some(ManagedSkillStatus::ContentDigestMismatch(metadata))),
                },
    ...
    fn recover_pending_backups(&self) -> Result<(), StorageError> {
        ...
        for entry in entries {
            ...
            let Some(name) = file_name
                .strip_prefix('.')
                .and_then(|value| value.split_once(".backup-").map(|(name, _)| name))
                .and_then(|name| SkillName::parse(name.to_owned()).ok())
            else {
                continue;
            };
            let destination = self.skills_root.join(name.as_str());
            if !destination.exists() {
                fs::rename(entry.path(), destination).map_err(StorageError::RestoreBackup)?;
            }
        }
...
pub enum ManagedSkillStatus {
    ...
    /// Metadata is valid but the rendered content differs from its recorded digest.
    ContentDigestMismatch(ManagedSkillMetadata),
...
pub enum StorageError {
    ...
    /// An interrupted replacement backup could not be restored.
    RestoreBackup(io::Error),
...
impl std::fmt::Display for StorageError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::RestoreBackup(error) => {
                write!(formatter, "failed to restore skill backup: {error}")
            }
...
impl std::error::Error for StorageError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            | Self::BackupSkill(error)
            | Self::RemoveBackup(error)
            | Self::ReadSkillsRoot(error)
            | Self::RestoreBackup(error) => Some(error),
...
mod tests {
    ...
    #[test]
    fn restores_an_interrupted_replacement_backup_when_storage_is_reopened() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("rust-skgen-interrupted-replacement-{unique}"));
        fs::create_dir(&root).unwrap();
        let name = SkillName::parse("existing-skill").unwrap();
        let skill = root.join(name.as_str());
        fs::create_dir(&skill).unwrap();
        fs::write(skill.join(SKILL_FILE_NAME), "# Previous skill\n").unwrap();
        fs::rename(&skill, root.join(".existing-skill.backup-interrupted")).unwrap();

        let reopened = TransactionalSkillCreator::new(&root);

        assert!(matches!(reopened.management_status(&name), Ok(Some(_))));
        assert_eq!(
            fs::read_to_string(skill.join(SKILL_FILE_NAME)).unwrap(),
            "# Previous skill\n"
        );
    ...
    #[test]
    fn keeps_a_published_skill_when_a_stale_backup_is_present() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("rust-skgen-stale-backup-{unique}"));
        fs::create_dir(&root).unwrap();
        let name = SkillName::parse("existing-skill").unwrap();
        let skill = root.join(name.as_str());
        fs::create_dir(&skill).unwrap();
        fs::write(skill.join(SKILL_FILE_NAME), "# Published skill\n").unwrap();
        let backup = root.join(".existing-skill.backup-stale");
        fs::create_dir(&backup).unwrap();
        fs::write(backup.join(SKILL_FILE_NAME), "# Previous skill\n").unwrap();
